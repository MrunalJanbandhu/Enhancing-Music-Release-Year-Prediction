...
class DimReduction(object):
    ...
    def get_indices(self, X):
        ...
        self.indices = np.array(idx)
        print(idx)
        return len(idx)

    def reduce(self, X):
        # return X
        return X[:, self.indices - 1]
...
def read_data():
    ...
    dim = DimReduction()
    NUM_FEATS = dim.get_indices(train.values)
    print(NUM_FEATS)
    ...
    train_input = dim.reduce(train_input)
    dev_input = dim.reduce(dev_input)
    test_input = dim.reduce(test_input)
